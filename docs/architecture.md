...
日期：2026-09-08。状态：架构基线 v1，已完成认证与单聊最小闭环。当前实现范围以 [项目说明](../README.md) 为准；架构图中的业务组件仍是目标设计，不代表已有完整功能或压测结果。
...
单聊创建使用排序后的双方 ID 生成唯一 directKey，防止两端同时创建两个会话。好友建立和解除的业务规则由 ContactService 管理；新建单聊和发言都要按规则检查。
...
图与本文给出的是业务实现契约。基础进程与协议探针逐步落入工程骨架，尚未完成聊天业务闭环或容量测试；具体已实现内容与运行命令见项目及各端 README。
