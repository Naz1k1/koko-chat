...
单文件最多 10 MiB，PNG/JPEG 图片最多 4,194,304 像素，每实例上传/下载共用 4 个并发名额。当前显示原图，没有缩略图或分片续传；尚无存储配额、病毒扫描、弃用上传定时清理。失败的本机副本和未绑定上传记录会保留。下载开始时校验权限，已开始的传输和用户此前保存的本地文件不会被远程撤回。RustFS 使用固定 RC.5 候选版本的单节点开发部署，未验证生产高可用或大规模性能。
