...
状态：架构基线，2026-09-08 已实现文本单聊及群聊的生产者、两级消费者、confirm/return、租约恢复、三档重试及死信。与 [整体架构](architecture.md) 配套；实际验证见 [单聊验收](chat-verification.md) 和 [群聊验收](group-verification.md)。
...
- 当前按单聊/群聊的有效成员逐设备发布网关任务，已验证三客户端群扇出；尚未按节点批量聚合。网关队列最多 5000 条、TTL 30 秒；通知过期、连接退出时由历史同步恢复。
...
- READ 提交后异步生成已读状态提示，复用 gateway.x 和临时网关队列。单聊通知双方在线设备，群聊只通知阅读者自身设备；消费时复核身份/周期并重查摘要。提示失败不影响 READ_ACK，由每 10 秒 HTTP 快照补齐；不混入 message_outbox 或消息重试/DLQ。
...
已读进度变化已使用可丢失的网关状态提示；后续好友申请结果、群成员变更可新增独立事件类型；图片处理、通知或搜索索引各自绑定独立队列。订阅者以数据库当前状态和事件版本为依据，重放不能重新授权已失效成员。
