...
状态：架构基线 v1，当前已实现认证、好友管理、文本单聊与群聊、账号独立 SQLite 缓存、历史补拉、展示分页与已读/未读同步，运行及验证范围见 [桌面工程说明](../apps/desktop/README.md) 和 [单聊验收](chat-verification.md)。本文描述完整目标；文件消息、全文检索等仍为后续实现契约，与 [整体架构](architecture.md) 和 [RabbitMQ 设计](messaging.md) 配套。
