...
DELETE FROM auth_session WHERE user_id IN (SELECT id FROM app_user WHERE account IN ('${KOKO_CHAT_TEST_ACCOUNT_PREFIX}a','${KOKO_CHAT_TEST_ACCOUNT_PREFIX}b'));
DELETE FROM app_user WHERE account IN ('${KOKO_CHAT_TEST_ACCOUNT_PREFIX}a','${KOKO_CHAT_TEST_ACCOUNT_PREFIX}b');
SQL
}
trap cleanup EXIT
