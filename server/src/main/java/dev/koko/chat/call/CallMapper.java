...
public interface CallMapper {
    String COLS="id,conversation_id,caller_id,callee_id,caller_session,callee_session,state,reason,expires_at";
    ...
    @Insert("INSERT INTO call_session(id,conversation_id,caller_id,callee_id,caller_session,expires_at) VALUES(#{id},#{conversationId},#{callerId},#{calleeId},#{callerSession},DATE_ADD(UTC_TIMESTAMP(3),INTERVAL 45 SECOND))") void create(Row row);
    ...
    @Update("UPDATE call_session SET state='ENDED',reason=#{reason},ended_at=UTC_TIMESTAMP(3) WHERE id=#{id} AND state<>'ENDED'") void end(String id,String reason);
